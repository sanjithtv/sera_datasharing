    ...
    public function show(Assessment $assessment)
{
    ...
    $rawData = \App\Models\AssessmentMasterData::where('assessment_id', $assessment->id)
        ->orderBy('entry_counter')
        ->get();

    // Group by entry_counter (one "row" = one group)
    $grouped = $rawData->groupBy('entry_counter');

    // Build headers from template keys (id → desc_en)
    $headers = $assessment->template->keys->pluck('short_code', 'id')->toArray();

    return view('modules.assessments.show', [
...
public function importData(Assessment $assessment)
{
    DB::beginTransaction();
...
/**
    ...
    {
        return view('modules.assessments.upload', compact('assessment'));
    }
